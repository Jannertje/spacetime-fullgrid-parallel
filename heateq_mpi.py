...
from source.linop import AsLinearOperator, CompositeLinOp, InvLinOp
...
class HeatEquationMPI:
    ...
    def __init__(self,
        ...
        if shared_comm.rank == 0:
            ...
            from source.ngsolve_helper import BilForm, KronBF, KronFES, LinForm
        ...
        elif wavelettransform == 'original':
        ...
        if precond == 'multigrid':
            ...
            self.C_j = [
                MultiGrid(mat,
                          hierarchy,
                          smoothsteps=smoothsteps,
                          vcycles=vcycles) for mat in self.Cinv_j
            ]
        else:
            assert (precond == 'direct')
            self.Kinv_x = InvLinOp(self.A_x)
            self.C_j = [InvLinOp(mat) for mat in self.Cinv_j]
